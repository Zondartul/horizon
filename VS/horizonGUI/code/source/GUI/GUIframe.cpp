...
void GUIframe::setProperty(string key, string val){
	if(key == "bgColor")			{setBgColor(		require(fromString<vec3>(val))	);}
	else if(key == "borderColor")	{setBorderColor(	require(fromString<vec3>(val))	);}
	else if(key == "noframe")		{noframe = 			require(fromString<bool>(val)	);}
