...
void GUIlabel::setProperty(string key, string val){
		 if(key == "textColor")				{setTextColor(			require(fromString<vec3>(val))				);}
	else if(key == "textfont")				{setTextFont(			require(fromString<font*>(val))				);}
	else if(key == "text")					{setText(val);}
	else if(key == "alignment_horizontal")	{alignment_horizontal = require(fromString<GUIe_alignment>(val));	}
	else if(key == "alignment_vertical")	{alignment_vertical = 	require(fromString<GUIe_alignment>(val));	}
	else if(key == "const_height")			{const_height = 		require(fromString<bool>(val));				}
