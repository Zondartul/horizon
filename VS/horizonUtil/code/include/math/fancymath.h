...
void DLLAPI_UTIL pack2D(vector<vec2> sizes,
			vec2 binsize,
			bool sort,
			vector<vec2> *results,
			vector<int> *packed,
...
vec2 DLLAPI_UTIL pack2DfindClosestPOT(vector<vec2> sizes, bool sort);

template<typename VT> 
VT interpolate_nearest(vector<pair<VT,float>> scatter, float pos)
{
...
template<typename VT> 
VT interpolate_linear(vector<pair<VT,float>> scatter, float pos)
{
	if (scatter.size() == 0) { throw runtime_error("nothing to interpolate"); }//{error("nothing to interpolate");}
	if (scatter.size() == 1){return scatter.front().first;}
	if (pos < scatter.front().second){return scatter.front().first;}
...
template<typename VT> 
VT interpolate_cubic(vector<pair<VT,float>> scatter, float pos)
{
	if (scatter.size() == 0) { throw runtime_error("nothing to interpolate"); } //{error("nothing to interpolate");}
	if (scatter.size() == 1){return scatter.front().first;}
	if (pos < scatter.front().second){return scatter.front().first;}
