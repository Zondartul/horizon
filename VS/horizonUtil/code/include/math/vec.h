...
*/
string DLLAPI_UTIL toString(quat q);
quat DLLAPI_UTIL toQuat(vec3 angleVec);
quat DLLAPI_UTIL slerpAngle(quat Q1, quat Q2, float angle); //moves at most <angle> radians from Q1 to Q2.
...
vec3 DLLAPI_UTIL clamp(vec3 A, vec3 min, vec3 max);
vec3 DLLAPI_UTIL operator/(vec3 A, float N);
vec3 DLLAPI_UTIL operator*(vec3 A, float N);

vec2 DLLAPI_UTIL clamp(vec2 A, vec2 min, vec2 max);
vec2 DLLAPI_UTIL operator/(vec2 A, float N);
vec2 DLLAPI_UTIL operator*(vec2 A, float N);
vec2 DLLAPI_UTIL normalizeSafe(vec2 V);
vec3 DLLAPI_UTIL normalizeSafe(vec3 V);
vec2 DLLAPI_UTIL toVec2(vec3 A);
vec3 DLLAPI_UTIL toVec3(vec2 A,float z = 0.f);

vec3 DLLAPI_UTIL setX(vec3 A, float x);
vec3 DLLAPI_UTIL setY(vec3 A, float y);
vec3 DLLAPI_UTIL setZ(vec3 A, float z);

float DLLAPI_UTIL cross(vec2 A, vec2 B);
vec3 DLLAPI_UTIL toVec3Angle(quat q);
vec3 DLLAPI_UTIL toVec3Angle(vec3 fwd);


struct DLLAPI_UTIL rect{
	vec2 start;
	vec2 end;
	vec2 size;
	rect();
...
rect DLLAPI_UTIL operator*(rect A, float N);
bool DLLAPI_UTIL operator==(rect A, rect B);


struct DLLAPI_UTIL AABB{
	vec3 start;
	vec3 end;
	vec3 size;
	AABB();
...
mat3 DLLAPI_UTIL xrotr(float ang);
mat3 DLLAPI_UTIL yrotr(float ang);
mat3 DLLAPI_UTIL zrotr(float ang);
