...
timer::~timer(){
	auto& timers = Gu->sysTimer->timers;//Gb->gs_timer->g_timers;
...
void simpletimer(function<void(timer *T)> F, int ticks_max){
	Gu->sysTimer->timers.push_back(new timer(F,ticks_max,0,1,1));
}
...
int getGameTicks(){
	auto& t = Gu->sysTimer->t;//Gb->gs_timer->g_t;
	return t;
}
float getGameTime(){
	auto& t2 = Gu->sysTimer->t2;//Gb->gs_timer->g_t2;
	return t2;
}
float getRealTime(){
	#ifdef USE_SDL
