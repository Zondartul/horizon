...
void GPUdriverKind::parseCommand(const renderCommand3 &rcmd){
    switch(rcmd.type){
        ...
        //----------------------------------------------------
            ...
            case(TEXTURE_UPLOAD):{
                texture *t = rcmd.t;
                if(wasRecentlyDeleted(t)){throw ex("accessing a deleted texture");}
                if(!t){throw ex("attempt to upload null texture\n");}
                ...
                if(t->bmp){
                    ...
                    int err = glGetError();
                    if(err){
                        if(err == GL_OUT_OF_MEMORY){throw ex("Error: GL_OUT_OF_MEMORY");}
                        string errstr = string()+"GL Error: "+err;
                        error(errstr.c_str());
                    }
                }else{
                    throw ex("no texture data to upload (%s)\n",t->name.c_str());
                }
            } break;
            ...
            case(TEXTURE_SELECT):{
                texture *t = rcmd.t;
                if(wasRecentlyDeleted(t)){throw ex("accessing a deleted texture");}
                if(!t){throw ex("attempt to select null texture\n");}
                GLuint handle = 0;
                //render_sys_data_texture rdata;
                if(t->bmp){
                    if(!bitmap_GPU_handles.count(t->bmp)){throw ex("image not uploaded: \"%s\"/%p\n",t->name.c_str(),t->bmp);}
                    handle = bitmap_GPU_handles[t->bmp];
                }else{
                    if(!texture_GPU_handles.count(t)){throw ex("texture not uploaded: %s\n",t->name.c_str());}
            ...
            case(RMODEL_UPLOAD):{
                rmodel *rm = rcmd.rm;
                if(wasRecentlyDeleted(rm)){throw ex("accessing a deleted rmodel");}
                if(!rm){throw ex("attempt to upload null rmodel\n");}
                ...
                if(
                    ...
                    throw ex("trying to upload incomplete rmodel");
                }
                num_rmodels++;
            ...
            case(RMODEL_RENDER):{
                ...
                switch(renderState.renderMode){
                    ...
                    case(3):
                        ...
                        break;
                    default:
                        throw ex("unknown render mode '%d'\n",renderState.renderMode);
                        break;
                }
            } break;
            ...
            case(RMODEL_DELETE):{
                rmodel *rm = rcmd.rm;
                if(wasRecentlyDeleted(rm)){throw ex("accessing a deleted rmodel");}
                if(!rm){throw ex("attempt to delete null rmodel\n");}
