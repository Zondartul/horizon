...
int octree_node::getOctant(AABB vaabb){
	...
	if(xn and yp and zn){res = 3; goto getoctExit;}
	if(xp and yp and zn){res = 4; goto getoctExit;}
	if(xn and yn and zp){res = 5; goto getoctExit;}
...
bool octree_node::canSplit(){
	return (volume.size.x > 1.f)
		and (volume.size.y > 1.f)
		and (volume.size.z > 1.f);
		//and visitors.size() > 1;
}
