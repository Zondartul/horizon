...
struct vec
{
	...
	vec operator * (vec B) const{return {x*B.x,y*B.y,z*B.z};}
	vec operator / (double b) const{return {x/b,y/b,z/b};}
	bool operator < (const vec B) const{return length() < B.length();}
	...
	vec norm() const{return *this/length();}
	double scalarProjection(vec B)const{return dot(B)/B.length();}
	vec vectorProjection(vec B)const{return B.norm()*scalarProjection(B);}
	double angle(vec B)const{return acos(dot(B)/(length()*B.length()))*180/M_PI;}
	double sgnAngle(vec B, vec ref) const// sign only possible when you know axis of rotation
