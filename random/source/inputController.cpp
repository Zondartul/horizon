...
void inputControllerKind::onEvent(eventKind event){
	if (event.type == EVENT_KEY_DOWN){
		...
		if(K == "T"){setPos(camera.pos+camera.forward()*camera.eyetrace(100)+(vec3f){0,0,0.5});if(character){character->toggleFly();character->toggleFly();}return;}
		if(K == "Left Shift"){speed = 0.14;flyspeed = 0.5;return;}
		if(K == "P"){physics = !physics;return;}
		if(K == "G"){toggleGravity();return;}
		if(K == "Numpad 7"){experiment1();}
