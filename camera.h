...
struct frustum{
	...
	vec getTri(int tri, int vert){ // this is really dumb. why can't we have a normal convex hull generator?
...
class cameraKind{
	...
	double getFrustumRight(){return tan(d2r(fov)/2.0)*znear;}
	double getFrustumTop(){return tan(d2r(fov*aspect)/2.0)*znear;}
	void setFrustum(){
		double top;
		double bottom;
		double left;
	...
	vec worldtoscreen(vec worldpos){ //algorithmical reversal of screentoworld
